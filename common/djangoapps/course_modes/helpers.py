...
def get_verified_track_links(language):
    ...
    if root_url and 'edx.org' in root_url:
        ...
        if support_url and 'support.edx.org' in support_url:
            ...
            track_comparison_url = urljoin(support_url, enabled_languages['en'], '/articles/360013426573-What-are-the-differences-between-audit-free-and-verified-paid-courses-')
            if language in enabled_languages:
                if language == 'es-419' or language == 'es':
                    track_comparison_url = urljoin(support_url, enabled_languages[language], '/articles/360013426573--Cu%C3%A1les-son-las-diferencias-entre-los-cursos-de-auditor%C3%ADa-gratuitos-y-verificados-pagos-')
            return {
